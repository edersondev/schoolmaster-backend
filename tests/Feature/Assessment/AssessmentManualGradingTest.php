...
final class AssessmentManualGradingTest extends TestCase
{
    use RefreshDatabase;

    public function test_teacher_records_manual_score_without_exposing_private_note(): void
    {
        ...
        $this->withHeaders($this->headers($teacher, $school))->postJson("/api/v1/questionnaire-responses/{$attempt->uuid}/grading", [
            'grading_outcomes' => [[
                ...
                'private_grading_note' => 'Internal note',
            ]],
        ])->assertOk()
            ->assertJsonPath('data.grading_status', 'graded')
            ->assertJsonPath('data.grading_outcomes.0.score', 88)
            ->assertJsonMissingPath('data.grading_outcomes.0.private_grading_note');

        $this->assertDatabaseHas('assessment_grading_outcomes', ['assessment_answer_id' => $answer->id, 'score' => 88]);
        $this->assertDatabaseHas('audit_events', ['event_type' => 'assessment.grading', 'school_id' => $school->id, 'outcome' => 'succeeded']);
    }

    public function test_invalid_manual_score_is_rejected(): void
    {
        [$school, $teacher, $attempt, $answer] = $this->context();

        $this->withHeaders($this->headers($teacher, $school))->postJson("/api/v1/questionnaire-responses/{$attempt->uuid}/grading", [
            'grading_outcomes' => [['answer_id' => $answer->uuid, 'status' => 'graded', 'score' => 101]],
        ])->assertUnprocessable();
    }

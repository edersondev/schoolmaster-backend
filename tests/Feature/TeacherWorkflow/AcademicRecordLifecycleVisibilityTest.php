...
final class AcademicRecordLifecycleVisibilityTest extends TestCase
{
    use RefreshDatabase;

    public function test_grade_lifecycle_restore_to_inactive_and_activation_dependency(): void
    {
        ...
        $this->withHeaders($headers)->patchJson("/api/v1/grades/{$grade->uuid}/correction", ['grade_value' => 82, 'correction_reason' => 'Correction note stays bounded.'])->assertOk()->assertJsonMissing(['request_payload']);
        ...
        $this->withHeaders($headers)->patchJson("/api/v1/grades/{$grade->uuid}/status", ['status' => 'active'])->assertConflict();
    }
